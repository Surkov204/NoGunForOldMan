...
namespace GameHealthSystem {
    ...
    public class health : MonoBehaviour, ISaveable
    {
        ...
        {
            return isPlayer ? "Player_Health" : null;
        }

        public Type GetSaveType() => typeof(HealthState);
        ...
        {
            var s = (HealthState)state;
            currentHealth = Mathf.Clamp(s.currentHealth, 0, startingHealth);
            Debug.Log($"[Health] Restored: {currentHealth}/{startingHealth} (Dead={dead})");
        }
        ...
        public void ResetState()
        {
            dead = false;
            currentHealth = startingHealth;
            Debug.Log("[Health] ResetState → full HP");
        }
